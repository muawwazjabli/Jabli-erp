...
setTimeout(() => {
    try {
        ...
        // Add a mutation observer specifically for class changes
        const observer = new window.MutationObserver(mutations => {
            mutations.forEach(m => {
                if (m.target.id === 'navFinance') {
                    console.log(`navFinance class changed: ${m.target.className}`);
                }
            });
        });
        observer.observe(document.getElementById('navFinance'), { attributes: true, attributeFilter: ['class'] });
